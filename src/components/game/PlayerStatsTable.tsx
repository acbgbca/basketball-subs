...
/**
 * Individual player row component to optimize rendering
 */
const PlayerRow: React.FC<{ player: Player; className?: string }> = React.memo(({ player, className }) => {
  const { activePlayers } = useGameStore();
  const { showFoulModal } = useUIStore();
  const stats = usePlayerStats(player.id);

  const handleFoulClick = () => {
    showFoulModal(player.id);
  };
...
/**
 * Enhanced player statistics table with real-time updates
 */
export const PlayerStatsTable: React.FC<PlayerStatsTableProps> = ({ className }) => {
  ...
  // Sort players: active players first, then by jersey number
    ...
    <Card className={className}>
      <Card.Header className="d-flex justify-content-between align-items-center">
        ...
        <div className="d-flex align-items-center gap-2">
          <Badge bg={activeCount > maxPlayers ? 'danger' : 'primary'}>
            {activeCount}/{maxPlayers} Active
          </Badge>
          <Badge bg="info">
            {game.players.length} Total
          </Badge>
        </div>
      </Card.Header>
      ...
      <Card.Body className="p-0">
        ...
        {activeCount > maxPlayers && (
          <div className="p-3 bg-danger text-white text-center">
            ...
            <div className="small">Maximum {maxPlayers} players allowed</div>
          </div>
        )}
